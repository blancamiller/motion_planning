            ...
            if count < NUMNODES:
                ...
                while foundNext == False:
                    ...
                    for p in nodes: #find nearest vertex
                        #check if this vertex is closer than previously selected closest
                        if dist(p.point,rand) <= dist(parentNode.point,rand): 
                            newPoint = step_from_to(p.point,rand)
                            # check if collision would occur with newly selected vertex
                            if collides(newPoint) == False:
                                #new point is not in collision, so update tnew vertex as best
                                parentNode = p 
                                foundNext = True
